...
class FlameListSeriesAdapter : PagingDataAdapter<FlameSeriesItem, FlameListSeriesAdapter.ViewHolder>(DIFF_CALLBACK) {
    class ViewHolder(private val binding: ListSeriesBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(data: FlameSeriesItem){
    ...
    companion object{
        val DIFF_CALLBACK = object : DiffUtil.ItemCallback<FlameSeriesItem>(){
            override fun areItemsTheSame(oldItem: FlameSeriesItem, newItem: FlameSeriesItem): Boolean {
                return oldItem == newItem
            }

            override fun areContentsTheSame(oldItem: FlameSeriesItem, newItem: FlameSeriesItem): Boolean {
